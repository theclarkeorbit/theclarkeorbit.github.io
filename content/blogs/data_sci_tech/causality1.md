...
##  1 (3) Mediator      0.910   0.909    -0.198 NA     NA    
##  2 (4) M-Bias        1.53   -0.487    19.7    2.40  -1.45 
##  3 (4) M-Bias       -1.76   -1.29     -4.10  -0.363  0.730
##  4 (1) Collider     -0.451   0.267    -0.156 NA     NA    
##  5 (2) Confounder   -0.506  -0.747     0.719 NA     NA    
##  6 (2) Confounder    0.445   1.08     -0.583 NA     NA    
##  7 (2) Confounder   -0.778  -1.22     -0.668 NA     NA    
##  8 (1) Collider      0.261  -0.668    -0.749 NA     NA    
##  9 (2) Confounder    1.31    0.748     1.24  NA     NA    
## 10 (2) Confounder    0.182   0.859     1.13  NA     NA
```
...
We can use the **d-separation criterion** (two variables are independent given your controls if no path between them can transmit information) to derive testable implications. The `dagitty` package does this automatically for us with the `impliedConditionalIndependencies` function.
...
rudder_dag |> 
  ...
  geom_dag_point(color = "#2D5016", size = 20, alpha = 0.4) +
...
- [Causal Inference in R](https://www.r-causal.org/) another free online textbook. 
- [Causal Inference: The Mixtape](https://mixtape.scunning.com/) by Cunningham (free, R and Stata code). 
- [`causalworkshop` R package](https://github.com/malcolmbarrett/causalworkshop) with tutorials. 
