...
It is worth stating that while the degree of randomness of returns on the price of a traded asset [can be tested](https://www.sciencedirect.com/science/article/pii/037842669390087T), the efficient market hypothesis is harder to test because of the so-called [joint hypothesis problem](http://finance.wharton.upenn.edu/~jwachter/fnce100/h11.pdf). How would we know if a market is inefficient ? We might look at all the information available, and find that the market behaves "abnormally" or "irrationally", given the available information. However to evaluate what a normal/abnormal return is, we need a model that connects available information to the price of an asset ([an asset pricing model](https://hbr.org/1982/01/does-the-capital-asset-pricing-model-work)). And therein lies the problem : even if we observe an "abnormal return", is the market inefficient, or is our asset pricing model wrong ? We cannot possibly know. 
